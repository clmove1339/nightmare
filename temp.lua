...
local function CompensateThrowable(cmd)
	...
	local grenade = ffi.cast('CBaseGrenade*', weapon);
	local weaponData = weapon->GetWeaponInfo();
	...
	if (type == 2) then
		...
		local base_vel = direction * (math.clamp(weaponData->flThrowVelocity * 0.9, 15, 750) * (grenade->m_flThrowStrength() * 0.7 + 0.3));
		...
		local throw_yaw = CalculateThrowYaw(target_vel, ctx.local_velocity, weaponData->flThrowVelocity, grenade->m_flThrowStrength());
	...
	if (type == 1) then
		cmd.viewangles.pitch += CalculateThrowPitch(cmd.viewangles:forward(), math.clamp(ctx.local_velocity.z, -120, 120), ctx.local_velocity, weaponData->flThrowVelocity, grenade->m_flThrowStrength());
    end
end
...
local function CalculateThrowYaw(wish_dir, vel, throw_velocity, throw_strength)
	...
	local cos_pitch = dir_normalized.Dot(wish_dir) / wish_dir.Q_Length();
